          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="text-muted-foreground text-xs flex items-center mb-1 hover:text-green-500">
                <CopyableEthText text={address || "Awqr...dx11"}></CopyableEthText>
              </div>
...
export default function PresaleComponent() {
  ...
  return (
    <div className="flex flex-col items-center justify-items-center gap-0">
      {tokens.map((token) => (
        <div key={token.id} className="p-2 w-full max-w-4xl gap-0">
          <TokenCard
            name={token.name}
            symbol={token.symbol.toUpperCase()}
            icon={token.image}
            marketCap={`$${(token.market_cap / 1000000).toFixed(2)}M`}
            pnl={`$${token.price_change_24h.toFixed(4)}`}
            pnlPercentage={`${token.price_change_percentage_24h.toFixed(2)}%`}
            address={token.id}
            showBoost={true}
            showPnl={true}
          />
